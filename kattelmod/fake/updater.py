...
from katcp.core import until_later
...
class IOLoopWarpClock(object):
    def __init__(self, start_time=None, update_period=None, ioloop=None):
        self.offset = (0.0 if start_time is None else
                       Timestamp(start_time).secs - time.time())
        ioloop = self.ioloop = ioloop or tornado.ioloop.IOLoop.current()
        self.ioloop.time = self.time
        self._update_period = update_period
        if update_period:
            self._updating = True
            ioloop.add_callback(periodic_update)
        else:
    ...
    def _periodic_update(self):
        while self._updating:
            before_update = self.clock.time()
            # Force the ioloop to run through a couple of times
            yield gen.moment
            yield gen.moment
            yield gen.moment
            after_update = self.clock.time()
            update_time = after_update - before_update
            remaining_time = self._update_period - update_time
            if remaining_time < 0:
                logger.warn("Update rate is too high: updates take "
                            "%g seconds but repeat every %g seconds" %
                            (update_time, self.period))
            # Warp the clock
            self.offset += max(remaining_time, 0)
